...
public struct Board {
    ...
    public let pos: [SquareStatus]
    /// Current player's turn
    public let turn: SquareStatus
    /// Opponent of the current turn (computed)
    public var opposite: SquareStatus { turn == .x ? .o : .x }
    ...
    public func move(_ location: Int) -> Board {
    ...
    public func safeMove(_ location: Int) -> Board? {
    ...
    public func generateWinningCombos() -> [[Int]] {
        let size = boardSize
    ...
    public var winningLine: (indices: [Int], winner: SquareStatus)? {
    ...
    public func minimax(_ board: Board, depth: Int, alpha: inout Int, beta: inout Int, maximizing: Bool, originalPlayer: SquareStatus) -> Int {
    ...
    public func findBestMove(_ board: Board) -> Int? {
    ...
    public func superiorminimax() -> Int? {
        return findSuperiorMove(timeLimit: 5.0)
    }
...
extension Board {
    ...
    public func bestMove(difficulty: AIDifficulty) -> Int {
        switch difficulty {
        case .easy:
            return easyMove()
        case .medium:
...
extension Board {
    ...
    public func advancedHeuristic(for player: SquareStatus) -> Int {
    ...
    /// `let`, chunki bir marta lazy tarzda hisoblanadi va keyin faqat o'qiladi —
    /// `var` bo'lsa bu global o'zgaruvchan holat bo'lib, data race'ga olib keladi.
    private static let zobristTable: [[Int]] = {
        var table: [[Int]] = []
    ...
    public func orderedLegalMoves() -> [Int] {
        let size = boardSize
        let center = size / 2
    ...
    public func superiorMinimaxCore(
        maxDepth: Int,
    ...
    public func findSuperiorMove(timeLimit: TimeInterval = 5.0) -> Int? {
        let startTime = Date()
...
// NOTE: A "future use" extension previously lived here holding
// `bitmaskRepresentation`, `transpositionTable`, `stateHash()`, `orderedMoves()`
// and `heuristicScore(for:)`. Every one of them returned a constant and had zero
// call sites in any of the three targets, so they were removed rather than
// carried across the module boundary. The real transposition table used by the
// search is `transTable`, local to the search itself.
